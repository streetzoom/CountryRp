Apartments = {}
Apartments.Starting = true      -- Enable or disable starting apartments 
Apartments.SpawnOffset = 30     -- How far under the map the apartment shell will spawn
Apartments.Locations = {        -- Create new apartment locations

    ["apartment6"] = {
        name = "apartment6",        -- The apartment name that saves in the database
        label = "Paleto house",     -- The label of the apartment (shown in preview)
        coords = {                  -- The apartment entrance location 
            enter = vector4(-346.79, 6223.49, 31.49, 28.86),
        },
        polyzoneBoxData = {         -- The polyzone box information for the entrance
            heading = 270,
            minZ = 28.5,
            maxZ = 31.0,
            debug = true,
            length = 1,
            width = 2,
            distance = 2.0,
            created = false
        }
    },

-- The apartments below are temporarily disabled due to unused!

--[[    ["apartment1"] = {
        name = "apartment1", 
        label = "South Rockford Drive", 
        coords = { 
            enter = vector4(-667.02, -1105.24, 14.63, 242.32),
        },
        polyzoneBoxData = { 
            heading = 245,
            minZ = 13.5,
            maxZ = 16.0,
            debug = true,
            length = 1,
            width = 3,
            distance = 2.0,
            created = false
        }
    }, 
    ["apartment2"] = {
        name = "apartment2",
        label = "Morningwood Blvd",
        coords = {
            enter = vector4(-1288.52, -430.51, 35.15, 124.81),
        },
        polyzoneBoxData = {
            heading = 124,
            minZ = 34.0,
            maxZ = 37.0,
            debug = true,
            length = 1,
            width = 3,
            distance = 2.0,
            created = false
        }
    },
    ["apartment3"] = {
        name = "apartment3",
        label = "Integrity Way",
        coords = {
            enter = vector4(269.73, -640.75, 42.02, 249.07),
        },
        polyzoneBoxData = {
            heading = 250,
            minZ = 40,
            maxZ = 43.5,
            debug = true,
            length = 1,
            width = 1,
            distance = 2.0,
            created = false
        }
    },
    ["apartment4"] = {
        name = "apartment4",
        label = "Tinsel Towers",
        coords = {
            enter = vector4(-619.29, 37.69, 43.59, 181.03),
        },
        polyzoneBoxData = {
            heading = 180,
            minZ = 41.0,
            maxZ = 45.5,
            debug = true,
            length = 1,
            width = 2,
            distance = 2.0,
            created = false
        }
    },
    ["apartment5"] = {
        name = "apartment5",
        label = "Fantastic Plaza",
        coords = {
            enter = vector4(291.517, -1078.674, 29.405, 270.75),
        },
        polyzoneBoxData = {
            heading = 270,
            minZ = 28.5,
            maxZ = 31.0,
            debug = true,
            length = 1,
            width = 2,
            distance = 2.0,
            created = false
        }
    }, ]]
}
